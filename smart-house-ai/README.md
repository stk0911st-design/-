...
次のような環境では、**自動実行を登録できません**（手動実行は問題なく使えます）。

- systemd が PID1 として動いていないコンテナ環境
- `crontab` コマンドが入っていない環境
- クラウド上の一時的な開発環境（一定時間で破棄されるもの）

`install_schedule.sh` は登録前にこれを検知して中断するので、
「登録できたつもりで実は動いていない」という事故は起きません。

確認方法：

```bash
ps -p 1 -o comm=        # systemd と出れば systemd 環境
echo $XDG_RUNTIME_DIR   # /run/user/... と出ればユーザーセッションあり
which crontab           # パスが出れば cron が使える
```

**自動実行は、実際に毎日電源が入っている社内のPCまたはサーバーで登録してください。**
...
| --- | --- |
| CSV / TSV | ✅ |
| Excel（.xlsx） | ✅ 自動でシートごとにCSVへ展開します |
| Markdown / テキスト / JSON | ✅ |
| Excel 旧形式（.xls） | ❌ .xlsx か CSV で保存し直してください |
| PDF / 画像 / Word | ❌ 対象外 |

**文字コードは自動判別します。UTF-8 でも Shift_JIS でも構いません。**
（UTF-8 / BOM付きUTF-8 / Shift_JIS・CP932 / EUC-JP / ISO-2022-JP に対応）

Excelから「CSV（コンマ区切り）」で保存した Shift_JIS のファイルも、そのまま置けます。

### 前処理の仕組み（元データは変更されません）
...
input/（元データ・絶対に変更しない）
      ↓ 読み取り専用で開く
  前処理：Excel→CSV展開 ／ Shift_JIS→UTF-8変換
      ↓
...
```
[09:30:12]   Excel展開: 在庫一覧.xlsx → 在庫一覧__2026年9月.csv（48行）
[09:30:12]   文字コード変換: 売上実績.csv（cp932 → UTF-8）
[09:30:12]   読み込み対象外: 契約書.pdf（対象外の形式です）
[09:30:12]   前処理結果: そのまま=3 変換=1 Excel展開=1 対象外=1
```

`読み込み対象外` と出たファイルは**AIに渡っていません**。
必要なデータであれば、CSV か .xlsx に変換して置き直してください。
...
**現在の設定：水曜・日曜が休み**（営業日は月・火・木・金・土の週5日）
...
変えたいときは `config/schedule.json` の1行を書き換えます。

```json
"holiday_weekdays": ["wed", "sun"]         ← 現在（水・日が休み）
"holiday_weekdays": ["sun"]                ← 水曜も営業にする
"holiday_weekdays": ["wed", "sat", "sun"]  ← 土曜も休みにする
```

書き換えたら**次回の実行から自動で反映**されます。スケジュールの再登録は不要です。
...
"deal_criteria": {
  ...
  "gross_margin_rate_basis": "purchase_price"  ← 粗利率の分母
}
```
...
`"gross_margin_rate_basis"` を `"sale_price"` にすると売却価格ベースになりますが、
**途中で変えると過去のレポートと数字が比較できなくなる**ので注意してください。

同じ案件でも、分母によって粗利率はこれだけ変わります。

| 例（想定売却 43,200,000円 / 取得 32,000,000円 / 粗利 4,679,452円） | 粗利率 |
| --- | --- |
| 仕入価格ベース（当社の定義） | 14.62% |
| 売却価格ベース | 11.76% |

「いくらなら買うか」の逆算も分母によって式が変わります。

| 定義 | 仕入価格の上限を出す式 |
| --- | --- |
| 仕入価格ベース（当社） | (想定売却価格 − コスト) ÷ (1 + 目標粗利率) |
| 売却価格ベース | 想定売却価格 ×(1 − 目標粗利率) − コスト |
