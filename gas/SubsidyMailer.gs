...
var SUBSIDIES = [
  ...
  },
  {
    ...
    note: '2026年10月1日から東京都1,280円・神奈川県1,279円へ改定（ともに54円引上げ）。'
      ...
      + '令和8年度予算は21億円（前年度15億円から増額）だが、予算に達し次第、締切前でも受付終了の可能性がある。',
  ...
  },
  {
    ...
    note: '人手不足解消のための省力化投資が対象。第8回の採択発表は2027年2月上旬予定。'
      + '第8回の変更点：労働生産性等の「基準年度」が「応募申請時の直近決算期」から'
      + '「補助事業が完了した日を含む事業年度の決算期」に変更され、'
      + '完了を待たずに行う「足下の賃上げ」が審査項目として明記された。',
  ...
  },
  {
    ...
    profiles: ['tokyo'],
    deadline: null,
    deadlineNote: '事前エントリー制。各回140社・全10回。応募多数の場合は抽選（現在の回次と受付期間を要確認）',
    amount: '最大264万円',
    rate: '定額',
    requirements: [
    ...
    note: '予定数を上回る申込があった場合は受付期間終了後に抽選。先着ではないため、'
      + '「早く出す」より「エントリー期間内に確実に出す」ことが重要。',
  ...
  {
    name: '事業環境変化に対応した経営基盤強化事業（賃上げ重点コース）',
    agency: '東京都中小企業振興公社',
    profiles: ['tokyo'],
    deadline: null,
    closed: true,
    deadlineNote: '令和8年度は2026年2月24日〜3月13日で終了。次回は令和9年度（2027年2月頃）の見込み（要確認）',
    amount: '上限800万円（上限600万円とする情報もあり要確認）',
    rate: '中小企業3/4以内・小規模企業4/5以内',
    requirements: [
      '都内の中小企業者であること',
      '賃上げ計画の策定・実施',
      'Jグランツによる電子申請（GビズIDプライムが事前に必要）'
    ],
    documents: ['申請書、賃上げ計画、見積書、決算書 ほか（募集要項を確認）'],
    note: '助成率3/4・上限800万円は都の制度では破格。賃上げがテーマなら最優先で狙う価値がある。'
      + '2027年2月の受付開始に向けて逆算で準備すること。事務局 03-4446-2560（平日9:00〜16:30）。',
    url: 'https://www.tokyo-kosha.or.jp/support/josei/jigyo/kankyo-juuten/index.html'
  },
  {
    name: '人材開発支援助成金',
    agency: '厚生労働省',
