...
| `NOISE_SENDERS` | （空） | 無条件で除外したい送信元を追加。カンマ区切りの部分一致 |
...
| 種別 | 判定 |
| --- | --- |
| 一斉配信・システム通知（求人サイト、SNS、Backlog 通知など） | 無条件で対象外。`MT_BLOCK_SENDERS` と `NOISE_SENDERS` で指定 |
| 送信専用アドレス（`noreply@` 系） | 案件語2つ以上、または至急語があれば対象として残す。返信下書きは作らず、本文から担当者アドレスを拾って報告する |
| それ以外 | 通常どおり判定 |
